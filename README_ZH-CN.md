...
没有项目内的 SOP / 控制平面时，强模型出的问题不只是「偶尔写错」。它是在**无界的歧义空间**里执行：

| 会发生什么 | 后果 |
|------------|------|
| 意图只活在聊天里 | 换会话 / 换模型就**忘**；同一条 MUST 被重谈或直接丢掉 |
| 很多路径「好像都能用」 | 跳过预览、旧写表脚本、双份状态 —— 交付**看起来完成了**，产品不变量在烂 |
| 你继续加提示词和守卫 | 纸面上错误面变小，**旁路面仍开着** —— 模型绕着栅栏走 |
| 任务做到一半换模型 | 可达动作往往**变宽**；之前收紧的约束带不到新执行者身上 |
| 没有吸收核对 | 「须预览再写表」从未变成代码里的**消费者** —— 文档与现实分叉 |

这才是设计的起点：**不是**「把模型管得更听话」，而是**让可游荡的空间在仓库里变得有限、可见、可编辑**——让下一个模型继承的是同一个世界。
